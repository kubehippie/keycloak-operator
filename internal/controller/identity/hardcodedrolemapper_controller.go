...
const hardcodedRoleMapperFinalizer = "keycloak-operator.webhippie.de/hardcodedrolemapper"

// HardcodedRoleMapperReconciler reconciles a HardcodedRoleMapper object.
type HardcodedRoleMapperReconciler struct {
	client.Client
	Scheme *runtime.Scheme
}

// +kubebuilder:rbac:groups=identity.keycloak-operator.webhippie.de,resources=hardcodedrolemappers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=identity.keycloak-operator.webhippie.de,resources=hardcodedrolemappers/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=identity.keycloak-operator.webhippie.de,resources=hardcodedrolemappers/finalizers,verbs=update

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
func (r *HardcodedRoleMapperReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := logf.FromContext(ctx)
	log.Info("Reconciling")

	instance := &identityv1alpha1.HardcodedRoleMapper{}
	...
	if !controllerutil.ContainsFinalizer(instance, hardcodedRoleMapperFinalizer) {
		controllerutil.AddFinalizer(instance, hardcodedRoleMapperFinalizer)
	...
	return r.reconcileHardcodedRoleMapper(ctx, instance, session, alias)
}

func (r *HardcodedRoleMapperReconciler) handleDeletion(ctx context.Context, instance *identityv1alpha1.HardcodedRoleMapper, session *controller.KeycloakSession, alias string) (ctrl.Result, error) {
	log := logf.FromContext(ctx)
	...
	controllerutil.RemoveFinalizer(instance, hardcodedRoleMapperFinalizer)
	if err := r.Update(ctx, instance); err != nil {
...
func (r *HardcodedRoleMapperReconciler) reconcileHardcodedRoleMapper(ctx context.Context, instance *identityv1alpha1.HardcodedRoleMapper, session *controller.KeycloakSession, alias string) (ctrl.Result, error) {
	log := logf.FromContext(ctx)

	desired := hardcodedRoleMapperToGocloak(instance, alias)
...
func hardcodedRoleMapperToGocloak(mapper *identityv1alpha1.HardcodedRoleMapper, alias string) gocloak.IdentityProviderMapper {
	return gocloak.IdentityProviderMapper{
		Name:                   gocloak.StringP(mapper.Spec.Name),
		IdentityProviderMapper: gocloak.StringP("oidc-hardcoded-role-idp-mapper"),
		IdentityProviderAlias:  gocloak.StringP(alias),
		Config: map[string]string{
			"role": mapper.Spec.Role,
		},
	}
}
