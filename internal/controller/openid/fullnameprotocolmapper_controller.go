...
const fullNameProtocolMapperFinalizer = "keycloak-operator.webhippie.de/fullnameprotocolmapper"

// FullNameProtocolMapperReconciler reconciles a FullNameProtocolMapper object.
type FullNameProtocolMapperReconciler struct {
	client.Client
	Scheme *runtime.Scheme
}

// +kubebuilder:rbac:groups=openid.keycloak-operator.webhippie.de,resources=fullnameprotocolmappers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=openid.keycloak-operator.webhippie.de,resources=fullnameprotocolmappers/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=openid.keycloak-operator.webhippie.de,resources=fullnameprotocolmappers/finalizers,verbs=update

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
func (r *FullNameProtocolMapperReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := logf.FromContext(ctx)
	log.Info("Reconciling")

	instance := &v1alpha1.FullNameProtocolMapper{}
	...
	if !controllerutil.ContainsFinalizer(instance, fullNameProtocolMapperFinalizer) {
		controllerutil.AddFinalizer(instance, fullNameProtocolMapperFinalizer)
	...
	return r.reconcileFullNameProtocolMapper(ctx, instance, session, idOfClient)
}

func (r *FullNameProtocolMapperReconciler) handleDeletion(ctx context.Context, instance *v1alpha1.FullNameProtocolMapper, session *controller.KeycloakSession, idOfClient string) (ctrl.Result, error) {
	log := logf.FromContext(ctx)
	...
	controllerutil.RemoveFinalizer(instance, fullNameProtocolMapperFinalizer)
	if err := r.Update(ctx, instance); err != nil {
...
func (r *FullNameProtocolMapperReconciler) reconcileFullNameProtocolMapper(ctx context.Context, instance *v1alpha1.FullNameProtocolMapper, session *controller.KeycloakSession, idOfClient string) (ctrl.Result, error) {
	log := logf.FromContext(ctx)

	desired := fullNameProtocolMapperToGocloak(instance)
...
func fullNameProtocolMapperToGocloak(mapper *v1alpha1.FullNameProtocolMapper) gocloak.ProtocolMapperRepresentation {
	return gocloak.ProtocolMapperRepresentation{
		...
		ProtocolMapper: gocloak.StringP("oidc-full-name-mapper"),
...
func (r *FullNameProtocolMapperReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.FullNameProtocolMapper{}).
		Named("openid-fullnameprotocolmapper").
		Complete(r)
}
