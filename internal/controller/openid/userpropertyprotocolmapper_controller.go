...
const userPropertyProtocolMapperFinalizer = "keycloak-operator.webhippie.de/userpropertyprotocolmapper"

// UserPropertyProtocolMapperReconciler reconciles a UserPropertyProtocolMapper object.
type UserPropertyProtocolMapperReconciler struct {
	client.Client
	Scheme *runtime.Scheme
}

// +kubebuilder:rbac:groups=openid.keycloak-operator.webhippie.de,resources=userpropertyprotocolmappers,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=openid.keycloak-operator.webhippie.de,resources=userpropertyprotocolmappers/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=openid.keycloak-operator.webhippie.de,resources=userpropertyprotocolmappers/finalizers,verbs=update

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
func (r *UserPropertyProtocolMapperReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := logf.FromContext(ctx)
	log.Info("Reconciling")

	instance := &v1alpha1.UserPropertyProtocolMapper{}
	...
	if !controllerutil.ContainsFinalizer(instance, userPropertyProtocolMapperFinalizer) {
		controllerutil.AddFinalizer(instance, userPropertyProtocolMapperFinalizer)
	...
	return r.reconcileUserPropertyProtocolMapper(ctx, instance, session, idOfClient)
}

func (r *UserPropertyProtocolMapperReconciler) handleDeletion(ctx context.Context, instance *v1alpha1.UserPropertyProtocolMapper, session *controller.KeycloakSession, idOfClient string) (ctrl.Result, error) {
	log := logf.FromContext(ctx)
	...
	controllerutil.RemoveFinalizer(instance, userPropertyProtocolMapperFinalizer)
	if err := r.Update(ctx, instance); err != nil {
...
func (r *UserPropertyProtocolMapperReconciler) reconcileUserPropertyProtocolMapper(ctx context.Context, instance *v1alpha1.UserPropertyProtocolMapper, session *controller.KeycloakSession, idOfClient string) (ctrl.Result, error) {
	log := logf.FromContext(ctx)

	desired := userPropertyProtocolMapperToGocloak(instance)
...
func userPropertyProtocolMapperToGocloak(mapper *v1alpha1.UserPropertyProtocolMapper) gocloak.ProtocolMapperRepresentation {
	return gocloak.ProtocolMapperRepresentation{
		...
		Config: map[string]string{
			"user.attribute":            mapper.Spec.UserProperty,
...
func (r *UserPropertyProtocolMapperReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&v1alpha1.UserPropertyProtocolMapper{}).
		Named("openid-userpropertyprotocolmapper").
		Complete(r)
}
